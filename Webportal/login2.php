<?php 
 ...
 if (isset($_POST['uname']) && isset($_POST['password'])) {
    ...
    if (empty($uname)) {
        ...
        header("Location: productsdb_login.php?error=User name is required");

        exit();

    }else if(empty($pass)){

        header("Location: productsdb_login.php?error=Password is required");
	...
	$gowno = mysqli_num_rows($result);
        ...
        if (mysqli_num_rows($result) === 1) {
            ...
            if ($row['ADMIN_NAME'] === $uname && $row['ADMIN_PASSWORD'] === $pass) {
                ...
                header("Location: productsdb_login.php?error=1Incorect User name or password");
